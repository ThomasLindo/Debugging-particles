...
void main()
{
	vec2 texOffset = o.texcoord * 10;
	texOffset = o.worldPos.xz;

	vec4 albedoColor = texture(uTexAlbedo, texOffset - uTime * 0.13);// + vec4(0.0, 0.2, 0.4, 1);
	albedoColor = mix(albedoColor, texture(uTexAlbedo, texOffset + uTime * 0.13 + vec2(0.5, 0)), 0.5);
	outColor.rgb = albedoColor.rgb * uSceneAmbient; 
	outColor.a = albedoColor.a;
	float noise = texture(uTexNoise, texOffset * 0.23 + uTime * 0.01).r;
	outColor.a *= noise * 1 + 0.5;
	//outColor
