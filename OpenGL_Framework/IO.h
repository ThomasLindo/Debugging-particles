...
namespace io
{
	...
	std::string addInclude(std::string data, const std::string &filepath);
...
#define PRINT_LOG(message, ...)			printf(message, ##__VA_ARGS__); printf("\n");

#define PRINT_LOG_ERROR(message, ...)	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x0C);\
										printf(message, ##__VA_ARGS__);	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x07); printf("\n");

#define PRINT_LOG_WARNING(message, ...)	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x0E); printf(message, ##__VA_ARGS__);\
										SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x07); printf("\n");

#define PRINT_LOG_SAFE(message, ...)	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x0A); printf(message, ##__VA_ARGS__);\
										SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x07); printf("\n");

#define __SAT_STRINGIFY(x) #x
#define __SAT_TOSTRING(x) __SAT_STRINGIFY(x)
#define AT "[" __FILE__ ":" __SAT_TOSTRING(__LINE__) "]"
...
#define SAT_ERROR_LOC(message, ...)		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x0C); printf("%s ", AT); \
										PRINT_LOG_ERROR(message, ##__VA_ARGS__)
...
#define SAT_DEBUG_LOG(message, ...)			__noop
#define SAT_DEBUG_LOG_ERROR(message, ...)	__noop	
#define SAT_DEBUG_LOG_WARNING(message, ...)	__noop
#define SAT_DEBUG_LOG_SAFE(message, ...)	__noop
